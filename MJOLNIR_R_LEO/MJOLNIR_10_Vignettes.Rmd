...
difflmeansmodelcomp <- function(listlmermodels){
  ...
  df_r <- purrr::reduce(l_r, inner_join) %>% as.data.frame()
...
grph <- dff_q %>%
  ...
  ggplot(aes(y=Value, x=Question, fill=V_Product)) +
  geom_violin()
grph
```
...
dff.means.comp %>%
  filter(!str_detect(name, "Indian|Black"), str_detect(name,"cigarettes")) %>%
  arrange(name) %>%
  select(MorallyWrong, CatchCovid, TransmitCovid) %>%
