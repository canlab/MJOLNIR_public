...
```{r}
aov(BDI_weighted ~ EDUCATION_1_TEXT, data=df_BDIpositive) %>% tukey_hsd()
ggplot(df_BDIpositive, aes(x = EDUCATION_1_TEXT, y = BDI_weighted)) + 
  geom_boxplot()
```
...
```{r}
aov(BDI_weighted ~ HH_INCOME_TEXT, data=df_BDIpositive) %>% tukey_hsd()
ggplot(df_BDIpositive, aes(x = HH_INCOME_TEXT, y = BDI_weighted)) + 
  geom_boxplot()
```
...
```{r}
anova <- aov(BDI_weighted ~ EXPGRP_TEXT, data=df_BDIpositive)
tukey <- TukeyHSD(anova)
print(tukey)
ggplot(data=df_BDIpositive, aes(EXPGRP_TEXT,BDI_weighted)) +
  geom_boxplot()
```


```{r}
df_BDIpositive$EXPGRP_TEXT <- gsub('-',' ',df_BDIpositive$EXPGRP_TEXT)

res.aov <- aov(BDI_weighted ~ EXPGRP_TEXT, data=df_BDIpositive)
pwc <- df_BDIpositive %>%
   tukey_hsd(BDI_weighted ~ EXPGRP_TEXT) 
pwc %>% add_xy_position(x = "EXPGRP_TEXT")

ggpubr::ggboxplot(df_BDIpositive,x = "EXPGRP_TEXT", y = "BDI") +
