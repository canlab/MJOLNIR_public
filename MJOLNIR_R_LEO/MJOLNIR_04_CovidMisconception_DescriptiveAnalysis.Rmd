...
```{r}
df_covmis$HH_INCOME_TEXT <- fct_recode(df_covmis$HH_INCOME %>% as.character,
"Less than $10,000"="1",
"$10,000 to $30,000"="2",
"$30,000 to $50,000"="3",
"$50,000 to $70,000"="4",
...
"$500,000 or more"="8")
table(df_covmis$HH_INCOME_TEXT, df_covmis$covqual_class) %>% cprop
table(df_covmis$HH_INCOME, df_covmis$covqual_class) %>% chisq.test()
```

```{r}
df_covmis$DOB_YEAR <- df$DOB_3 + 1900
df_covmis$DOB_YEAR_PERIODE <- df_covmis$DOB_YEAR %>% as.numeric() %>% cut(breaks = c(1945,1955,1965,1975,1985,1995,2005))
table(df_covmis$DOB_YEAR_PERIODE, df_covmis$covqual_class) %>% cprop
table(df_covmis$DOB_YEAR_PERIODE, df_covmis$covqual_class) %>% chisq.test()
```

```{r}
df_covmis$EDUCATION_1_TEXT <- fct_recode(df$EDUCATION_1 %>% as.character,
...
table(df_covmis$EDUCATION_1_TEXT, df_covmis$covqual_class) %>% cprop
table(df_covmis$EDUCATION_1_TEXT, df_covmis$covqual_class) %>% chisq.test()
table(df_covmis$EDUCATION_1_TEXT, df_covmis$covqual_class) %>% t.test()

table(df_covmis %>% filter(DOB_YEAR_PERIODE != "(1995,2005]") %>% pull(EDUCATION_1_TEXT),
      df_covmis %>% filter(DOB_YEAR_PERIODE != "(1995,2005]") %>% pull(covqual_class)) %>% cprop
table(df_covmis %>% filter(DOB_YEAR_PERIODE != "(1995,2005]") %>% pull(EDUCATION_1_TEXT),
      df_covmis %>% filter(DOB_YEAR_PERIODE != "(1995,2005]") %>% pull(covqual_class)) %>% t.test()
```
...
df_covmis$EDUCATION_2_TEXT <- fct_recode(df$EDUCATION_1 %>% as.character,
          ...
          "Graduate degree"="7",
          "Graduate degree"="8")
table(df_covmis %>% filter(DOB_YEAR_PERIODE != "(1995,2005]") %>% pull(EDUCATION_2_TEXT),
      df_covmis %>% filter(DOB_YEAR_PERIODE != "(1995,2005]") %>% pull(covqual_class)) %>% cprop
table(df_covmis %>% filter(DOB_YEAR_PERIODE != "(1995,2005]") %>% pull(EDUCATION_2_TEXT),
      df_covmis %>% filter(DOB_YEAR_PERIODE != "(1995,2005]") %>% pull(covqual_class)) %>% t.test()
```
...
```{r}
df_covmis$SEX_TEXT <- fct_recode(df$SEX %>% as.character,
          "Male"="1",
          "Female"="2",
          "Transgender"="3",
          "Other"="4")
table(df_covmis$SEX_TEXT,df_covmis$covqual_class) %>% cprop
table(df_covmis$SEX_TEXT,df_covmis$covqual_class) %>% chisq.test()
table(df_covmis$SEX_TEXT,df_covmis$covqual_class) %>% t.test()
```
